...
/**
 * @brief ROS1 facade over the DJI Payload SDK platform and module APIs.
 *
 * The wrapper owns the PSDK lifecycle and exposes two complementary access
 * paths: strongly typed ROS services/topics for common workflows and the
 * JSON dispatcher for broader PSDK 3.13 coverage.  PSDK callbacks publish
 * their data back into ROS messages/topics through the implementation file.
 */
namespace psdk_ros1
{

/**
 * Coordinates ROS endpoints, PSDK platform handlers, module lifecycle, and
 * safety-gated command dispatch.  `start()` is the forward initialization
 * sequence; `shutdown()` is idempotent cleanup for the reverse sequence.
 */
...
private:
  // Parameters are loaded from the node's private namespace.  Credential
  // fields are kept as runtime configuration and are never hard-coded here.
  struct Params
  {
    std::string app_name;
    std::string app_id;
    ...
    // Planner limits use SI units except max_yaw_rate, which is degrees/s to
    // match the public YAML/ROS configuration contract.
    ...
    // Gimbal feedback selects a payload slot (1..7) and a bounded PSDK topic
    // frequency; invalid values are normalized by loadParams().
  ...
  // A dispatcher result is copied into both the ROS service response and the
  // corresponding result topic so callers have one consistent result schema.
  struct ApiResult
  {
    bool success;
  ...
  // Startup/teardown stages.  The implementation registers the platform
  // boundary before initializing core, then initializes selected modules.
  bool loadParams();
  ...
  // Strongly typed ROS service endpoints for the common camera, gimbal, core,
  // and flight-controller workflows.
  ...
  // ROS subscriber callbacks translate topic messages into dispatcher calls or
  // update the planner/mission state consumed by their timers.
  ...
  // JSON API routing is split by PSDK module family to keep the public entry
  // point small while preserving one common request/result contract.
  ...
  // Publication helpers are the output boundary from PSDK/ROS callbacks to
  // the observable ROS state, result, event, and JSON topics.
  ...
  // Planner and mission helpers run from ROS timers/callbacks and keep command
  // conversion separate from the PSDK dispatcher branches.
  ...
  // These small parsers intentionally implement the package's lightweight
  // JSON contract without introducing a second JSON dependency.
  ...
  // ROS handles and the private namespace used for runtime configuration.
  ros::NodeHandle nh_;
  ...
  // Lifecycle and planner state.  These flags are updated by ROS callbacks
  // and timers; PSDK callback data is published through the global bridges in
  // the implementation and is not stored in these fields.
