...
bool PsdkRos1Wrapper::registerPlatformHandlers()
{
  ...
  uart_handler.UartGetDeviceInfo = HalUart_GetDeviceInfo;
                                                                     ...
                                                                     const std::string& request_json)
{
  ...
  if (function == "set_go_home_altitude") {
    ...
    rc = DjiFlightController_SetHorizontalVisualObstacleAvoidanceEnableStatus(jsonBool(request_json, "enabled", true));
  } else if (function == "get_horizontal_visual_obstacle_avoidance") {
    bool enabled = false;
    rc = DjiFlightController_GetHorizontalVisualObstacleAvoidanceEnableStatus(&enabled);
